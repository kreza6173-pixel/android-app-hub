...
def get_changelog_grouped(days=30, per_source_limit=MAX_CHANGELOG_PER_SOURCE):
    """Every entry per source within the last `days` days — never truncated
    across sources, so a noisy source can't push a quiet source's history
    out of view. Returns {source_id: [entries newest-first]}."""
